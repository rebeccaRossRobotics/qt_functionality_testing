...
RRJoystick::RRJoystick(QWidget *parent, Qt::WindowFlags f)
: QWidget(parent, f),
    m_x(0), m_y(0),
...
float RRJoystick::x() const
{
    return m_x;
}

float RRJoystick::y() const
{
    return m_y;
}

void RRJoystick::setX(float value)
{
    m_x = constrain(value, -1.f, 1.f);

    qreal radius = (m_bounds.width() - m_knopBounds.width()) / 2;
    m_knopBounds.moveCenter(
        QPointF(
            m_bounds.center().x() + m_x * radius, m_knopBounds.center().y()));

    update();
    emit xChanged(m_x);
}

void RRJoystick::setY(float value)
{
    ...
    m_knopBounds.moveCenter(
        QPointF(
            m_knopBounds.center().x(), m_bounds.center().y() + m_y * radius));

    update();
    emit yChanged(m_y);
}

void RRJoystick::removeXAnimation()
{
...
void RRJoystick::removeYAnimation()
{
...
void RRJoystick::setAlignment(Qt::Alignment f)
{
    m_alignment = f;
}

void RRJoystick::resizeEvent(QResizeEvent *event)
{
    Q_UNUSED(event)
    ...
    m_knopBounds.moveCenter(
        QPointF(
            m_bounds.center().x() + m_x * radius, m_bounds.center().y() - m_y * radius));
}

void RRJoystick::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
    ...
    painter.drawLine(
        QPointF(
            m_bounds.center().x() + m_bounds.width() * 0.35,
            m_bounds.center().y()),
...
void RRJoystick::mousePressEvent(QMouseEvent *event)
{
...
void RRJoystick::mouseReleaseEvent(QMouseEvent *event) {
    Q_UNUSED(event)
...
void RRJoystick::mouseMoveEvent(QMouseEvent *event)
{
    if (!knopPressed) {
        return;
    }
